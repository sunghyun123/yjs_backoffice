...
> 현재 상태: **Phase 0-4 구현·자동 검증 완료, 실계정·Tailscale 인수 시험 대기**
...
운영에서는 위키 증분 동기화가 `work_log`를 한 번만 공유 SQLite 색인으로 가져오고, 위키 검색과 대시보드가 같은 값을 파생해 사용합니다. 대시보드의 실행·자동시작·Tailscale Serve·인수 시험은 [배포 체크리스트](./배포_체크리스트.md)를 따릅니다.
